/*
  ----------------------------------------------------------------------------
  Filename    : r1_drive.ino
  Created on  : April 12, 2025
  Description : This code enables omni-directional drive control of a 3-wheeled
                robot using a PS4 controller via Bluetooth dongle on Teensy 4.1.
                Includes encoder feedback and PID speed control for precise motion.
                Communication also established via Serial8 for debugging or display.
  ...
  | Encoder 2         | 38 (A)           | 37 (B)           |
  | Encoder 3         | 40 (A)           | 39 (B)           |
  | Motor 1 (PWM+DIR) | 4 (PWM)          | 6 (DIR)          |
  | Motor 2 (PWM+DIR) | 5 (PWM)          | 7 (DIR)          |
  | Motor 3 (PWM+DIR) | 3 (PWM)          | 2 (DIR)          |
  | Serial8           | TX8/RX8          | (used for debug) |
  --------------------------------------------------------------------------
*/
...
void drive_pid() {
  ...
  if (isJoystick == true) {
    ...
    int leftStickX = joystick1.getAxis(2);
    leftX = map(leftStickX, 0, 255, -100, 100);

    // Right Stick values (axes 2 and 5)
    int rightStickX = joystick1.getAxis(0);
    x = map(rightStickX, 0, 255, -100, 100);

    int rightStickY = joystick1.getAxis(1);
