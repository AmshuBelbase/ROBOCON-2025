...
def zed2_process(shared):
    ...
    class Shooting:
        def __init__(self):
            ...
            self.CONST = 1067.6 #Calibrated constant
            self.distance = 0
        
        def get_hoop(self, frame):
            #run yolo inference
            self.frame = frame
            self.results = self.model.predict(source=self.frame, conf=self.CONFIDENCE_THRESHOLD, save=False, verbose=False)
        ...
        def process_hoop(self):
            ...
            for res in self.results:
                for box in res.boxes:
                    ...
                    self.x_center, self.y_center, width, height = box.xywh[0].cpu().numpy()
                    self.x_center, self.y_center, width, height = float(self.x_center), float(self.y_center), float(width), float(height)
                    ...
                    cv2.rectangle(self.frame, (int(xmin), int(ymin)), (int(xmax), int(ymax)), (255, 0, 0), 2)
                    cv2.line(self.frame, (frame_width//2, frame_height), (int(self.x_center), int(self.y_center)), (0, 0, 255), 2)
        ...
        def get_distance_then_rpm(self):
            ...
            coefficients = np.polyfit(distances,rpm ,2)
            shoot_rpm =  coefficients[0] * self.distance**2 + coefficients[1] * self.distance + coefficients[2]
    ...
    while True:
        try:
            ...
            while True:
                ...
                count_missed_mode = {"Passing": 1, "Shooting":3}

                # Get Rotational RPM based on the offset and required Mapping
                if (offset < (err_off + shift_x)) or (offset > (-err_off + shift_x)):
                    shared["pwm"] = 0
                    count_missed = 0
                elif offset == 0:
                    count_missed += 1
                    if count_missed >= count_missed_mode[shared["turn_mode"]]:
                        count_missed = 0
                        shared["pwm"] = 0
                elif (offset > (err_off + shift_x)) or (offset < (- err_off + shift_x)):
                    count_missed = 0
                    ...
                    thresholds = [
                        ...
                        (int(center*0.4), pwm_thres[1]),
                        (int(center*0.2), pwm_thres[0])
                    ]
                    thres_mins = {pwm_thres[3]: 0, pwm_thres[2]: 0, pwm_thres[1]: 0, pwm_thres[0]: 0}
                ...
                frame = cv2.line(frame, (frame_width//2 - err_off + shift_x, 0), (frame_width//2 - err_off + shift_x, frame_height), (255,0,255), 1)
                frame = cv2.line(frame, (frame_width//2 + err_off + shift_x, 0), (frame_width//2 + err_off + shift_x, frame_height), (255,255,0), 1)
                # frame= cv2.line(frame, (frame_width//2, 0), (frame_width//2,frame_height), (255,255,255), 1)
