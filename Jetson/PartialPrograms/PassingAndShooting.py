...
def zed2_process(shared):
    ...
    class Shooting:
        def __init__(self,confidence_threshold,basket_width_cm,const):
            self.confidence_threshold=confidence_threshold
            self.basket_width_cm=basket_width_cm
            self.const=const
            self.model=YOLO(r'Hoop_PT/n_hoop.pt')
            self.hoop_distance=0
            self.mapped_pwm=0
            self.bldc_rpm=0
        ...
        def target_locking_pwm(self,frame):
            frame_height, frame_width = frame.shape[:2]  
            threshold = 0.01
            right_threshold = int(frame_width / 2.0 + frame_width * threshold)
            left_threshold = int(frame_width / 2.0 - frame_width * threshold) 
            
            results=self.model.predict(source=frame, conf=self.confidence_threshold, save=False, verbose=False)
            det = False
            for res in results:
                for box in res.boxes:
                    ...
                    self.hoop_distance = unit_length * self.const
                    print(f"Distance to basket: {self.hoop_distance:.2f} cm")
                    ...
                    if right_threshold < x_center or x_center < left_threshold:
                        ...
                        self.mapped_pwm = map_range(abs(error_offset), 0, (frame_width//2),0, 1.0)
                        self.mapped_pwm=ease_out_expo(self.mapped_pwm)
                        self.mapped_pwm = map_range(abs(self.mapped_pwm), 0.0, 1.0,10,80)
                        self.mapped_pwm = -self.mapped_pwm if error_offset>0 else self.mapped_pwm
                        self.mapped_pwm=int(self.mapped_pwm)
                        print("PWM for target locking: ",self.mapped_pwm)
                        #Update values in shared dictionary
                        # shared['pwm'] = mapped_pwm
                        # shared['distance'] =self.hoop_distance
                    else:
                    ...
                    cv2.putText(frame, f"Dist: {self.hoop_distance:.2f}", (500, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 3)
        ...
        def get_RPM(self):
            ...
            coefficients = np.polyfit(distances,rpm ,2)
            self.bldc_rpm = (coefficients[0] * self.hoop_distance**2 + coefficients[1] * self.hoop_distance + coefficients[2])
            return self.bldc_rpm
    ...
    class Passing:
        def __init__(self):
            ...
            self.calib_data = np.load(r'/home/ghost/Documents/Siddhant/R2/MultiMatrixzed21080.npz')  # Replace with your filename
    ...
    pass_obj = Passing()
    shooting_obj=Shooting(0.25,48.5,1067.55)#confidnence,Basket_width,CONSTANT
    count_missed = 0
        ...
        try:
            ...
            while True:
                ...
                if shared["turn_mode"] == "Passing":
                    ...
                    FRAME_SKIP = 5
                    offset = 0
                    shared["pwm"]=shooting_obj.target_locking_pwm(frame=frame)
                    shared["bldc_rpm"] = shooting_obj.get_RPM()
                else:
                    offset = 0
                    shared["bldc_rpm"] = 0
