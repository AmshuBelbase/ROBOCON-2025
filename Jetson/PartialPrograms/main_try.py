...
R = {
    1: {
        ...
        'shoot': {
            ...
            'pwm_thres': [95, 130, 160, 190, 200],
            'shift_x': 0,
            'err_off': 6
        }
    },
    2: {
        'cam_file': r"CameraCalibration/MultiMatrixLenevo.npz",
        'pass': {
...
def zed2_process(shared):
    ...
    class ZED2_feed:
        def __init__(self):
            ...
            # Use HD1080, HD720 opr HD1200 video mode, depending on camera type.
    ...
    class Passing:
        def __init__(self):
            distances = R[bot]['pass']['distances']
            rpm = R[bot]['pass']['rpm']
            self.coefficients = np.polyfit(distances, rpm, 2)

            # === Tag parameters ===
            # Real-world tag size in centimeters (e.g., 15 cm)
            self.tag_size = 0.305
            # constants for passing (apriltag)
            # Replace with your filename
            self.calib_data = np.load(R[bot]['cam_file'])
        ...
        def get_tags(self, frame):
            self.frame = frame
            # cv2.undistort(frame, camera_matrix, dist_coeffs)
            self.frame_undistorted = self.frame
            self.gray = cv2.cvtColor(
                self.frame_undistorted, cv2.COLOR_BGR2GRAY)
        ...
        def process_tags(self):
            ...
            for tag in self.tags:
                ...
                cv2.circle(self.frame, (frame_width//2,
                           frame_height), 6, (255, 255, 0), -1)
                cv2.line(self.frame, (frame_width//2, frame_height),
                         center, (0, 255, 255), 2)
                ...
                cube_points_3d -= np.array([self.tag_size/2,
                                           self.tag_size/2, 0])
                ...
                cube_points_2d, _ = cv2.projectPoints(
                    cube_points_3d, rvec, tvec, self.camera_matrix, self.dist_coeffs)
                ...
                back_face_center_2d, _ = cv2.projectPoints(
                    np.array([back_face_center_3d]
                             ), rvec, tvec, self.camera_matrix, self.dist_coeffs
                )
                back_face_center_2d = tuple(
                    back_face_center_2d[0][0].astype(int))
                cv2.line(frame, (frame_width//2, frame_height),
                         back_face_center_2d, (0, 0, 255), 2)
        ...
        def get_distance_then_rpm(self):
            ...
            pass_rpm = (self.coefficients[0] * distance_pass**2 +
                        self.coefficients[1] * distance_pass + self.coefficients[2])*1.02
    ...
    class Shooting:
        def __init__(self):
            # Load YOLO model
            self.model = YOLO(r'Hoop_PT/n_hoop.pt')
            # Constants for hoop distance
            self.CONFIDENCE_THRESHOLD = 0.25
            self.FRAME_SKIP = 5
            self.BASKET_WIDTH_CM = 47.2
            self.CONST = 487  # Calibrated constant
            self.distance = 0
            distances = R[bot]['shoot']['distances']
            rpm = R[bot]['shoot']['rpm']
            self.coefficients = np.polyfit(distances, rpm, 2)

        def get_hoop(self, frame):
            # run yolo inference
            self.frame = frame
            self.results = self.model.predict(
                source=self.frame, conf=self.CONFIDENCE_THRESHOLD, save=False, verbose=False)
        ...
        def process_hoop(self):
            # Flag to check i
            # f Basket detected or not
            self.det = False
            self.distance = 0
            self.offset = 0
            ...
            for res in self.results:   
                ...
                for box in res.boxes:
                    ...
                    class_name = self.results[0].names.get(class_id, "Unknown")
                    ...
                    self.x_center, self.y_center, width, height = box.xywh[0].cpu(
                    ).numpy()
                    self.x_center, self.y_center, width, height = float(
                        self.x_center), float(self.y_center), float(width), float(height)

                    frame_height, frame_width = self.frame.shape[:2]

                    if self.y_center > frame_height//2 or ymin < 10:
                        continue

                    self.det = True 
                    self.offset = self.x_center - frame_width // 2

                    if abs(self.offset)<min_offset:
                        min_offset= abs(self.offset)
                        closest_box = {
                            "x_centre": self.x_center,
                            "y_centre": self.y_center,
                            "width": width,
                            "height":height,
                ...
                if closest_box: 
                    ...
                    xmin = closest_box["x_center"] - (width // 2)
                    ymin = closest_box["y_center"] - (height // 2)
                    xmax = closest_box["width"] + (width // 2)
                    ymax = closest_box["height"] + (height // 2)
                        


                    # Calculate the distance to basket
                    unit_length = self.BASKET_WIDTH_CM / width
                    self.distance = unit_length * self.CONST
                    # print(f"Distance to basket: {self.distance:.2f} cm")

                    self.offset = self.x_center - frame_width // 2

                    self.confidence = round(box.conf[0].item(), 3)
                    cv2.rectangle(self.frame, (int(xmin), int(ymin)),
                                  (int(xmax), int(ymax)), (255, 0, 0), 2)
                    cv2.line(self.frame, (frame_width//2, frame_height),
                             (int(self.x_center), int(self.y_center)), (0, 0, 255), 2)

        def get_distance_then_rpm(self):
            if self.det == False:
                text = f"Distance {0} RPM {0}"
                position = (0 + 40, 0 + 20)
                self.draw_text_with_background(
                    text=text, org=position,
                    font=cv2.FONT_HERSHEY_SIMPLEX,
            ...
            shoot_rpm = self.coefficients[0] * self.distance**2 + \
                self.coefficients[1] * self.distance + self.coefficients[2]

            text = f"Distance {self.distance:.2f} RPM {shoot_rpm}"
            position = (0 + 40, 0 + 20)
            self.draw_text_with_background(
                text=text, org=position,
                font=cv2.FONT_HERSHEY_SIMPLEX,
        ...
        try:
            ...
            while True:
                ...
                if shared["turn_mode"] == "Passing" and bot == 2:
                    ...
                    if offset is not None and abs(offset)<20:
                        shared["bldc_rpm"] = shoot_obj.get_distance_then_rpm()
                        shared["ready_to_shoot"]=True
                    else:
                        shared["bldc_rpm"] = 0
                        shared["ready_to_shoot"] = False

                    
                    offset = shoot_obj.get_alignment_offset()
                    shared["bldc_rpm"] = shoot_obj.get_distance_then_rpm()
                    if text_mode:
                        number = 0
                        with open('PartialPrograms/rpm.txt', 'r') as file:
                ...
                if (center+offset) > (shifted_center - err_off) and (center+offset) < (shifted_center + err_off):
                    ...
                    count_missed += 1
                    if count_missed >= count_missed_mode[shared["turn_mode"]]:
                        count_missed = 0
                        shared["pwm"] = 0
                ...
                try:
                    ...
                    if data == b'\x01':
                        ...
                        response = struct.pack(
                            ...
                            int(shared['r1']), int(shared['l1']), int(shared['cross']), int(
                                shared['square']), int(shared['circle']),
                            int(shared['triangle']), int(shared['touch_button']), int(
